...
import {
  selectAllPublicOffers,
  selectPublicOffers,
} from '../../../zustandStore/selectors';
...
export const MarketTableAdmin: FC = () => {
  const [offersIsLoading, refreshOffers] = useRootStore((state) => [
    state.interfaceIsLoading,
    state.refreshOffers,
  ]);
  ...
  const [allOffers, offersLoading] = useRootStore((state) => [
    state.offers,
  ...
  const { offers } = useTypedOffers(
    USER_ROLE.ADMIN ? allPublicOffers : publicOffers
  );
