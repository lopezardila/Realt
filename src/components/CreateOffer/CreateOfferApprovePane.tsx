...
export const CreateOfferApprovePane = ({
  ...
  const [checkIfApproveNeeded, setCheckIfApproveNeeded] =
  ...
  const propertie = useMemo(() => {
    return properties.find(
      (prop) => prop.contractAddress.toLowerCase() === token.toLowerCase()
    );
  }, [properties]);
    ...
    <Flex className={classes.container} justify={'space-between'}>
      <Flex direction={'column'}>
        ...
        <Text fs={'italic'} fw={500} c={'gray'}>
          {amount ? amountInEther : <Skeleton height={35} width={'100%'} />}
        </Text>
      </Flex>
      <Button
        color={'green'}
